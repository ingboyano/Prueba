...
from wtforms import (StringField, PasswordField, IntegerField, SubmitField,
                     TextAreaField, EmailField, SelectField, BooleanField)
...
class VMForm(FlaskForm):
    ...
    requester_name = StringField('Nombre del Solicitante',
    ...
    # Administradores
    primary_admin_name = StringField('Nombre del Administrador Principal',
                                     validators=[Optional()])
    primary_admin_email = EmailField(
        'Correo del Administrador Principal',
        validators=[
            Optional(),
            Email(message='Por favor ingrese un correo válido')
        ])
    secondary_admin_name = StringField('Nombre del Administrador Secundario',
                                       validators=[Optional()])
    secondary_admin_email = EmailField(
        'Correo del Administrador Secundario',
        validators=[
            Optional(),
            Email(message='Por favor ingrese un correo válido')
        ])
    ...
    flavor = SelectField(
        'Tipo de Flavor',
        choices=[('custom', 'Personalizado'),
                 ('small', 'Pequeño (1 CPU, 2 GB RAM, 20 GB Disco)'),
                 ('medium', 'Mediano (2 CPU, 4 GB RAM, 40 GB Disco)'),
                 ('large', 'Grande (4 CPU, 8 GB RAM, 80 GB Disco)')],
        default='custom')

    cpu_count = IntegerField('Número de CPUs',
                             validators=[
                                 Optional(),
                                 NumberRange(
                                     min=1,
                                     message="Debe tener al menos 1 CPU")
                             ])
    ram_amount = IntegerField(
        'Cantidad de RAM (GB)',
        validators=[
            Optional(),
    ...
    operating_system = SelectField(
        'Sistema Operativo',
        choices=[('', 'Seleccione un sistema operativo'),
    ...
    physical_location = StringField('Ubicación Física',
                                    validators=[Optional()])
    virtual_host = StringField('Host Virtual / Infraestructura',
                               validators=[Optional()])

    # Configuración de respaldo
    backup_required = BooleanField('Requiere Respaldo', default=False)
    backup_frequency = SelectField('Frecuencia de Respaldo',
                                   choices=[
                                       ('', 'No aplica'), ('daily', 'Diario'),
                                       ('weekly', 'Semanal'),
                                       ('monthly', 'Mensual'),
                                       ('custom',
                                        'Personalizada (especificar en notas)')
                                   ],
                                   validators=[Optional()])

    # VLANs
    vlans = StringField('VLANs (separadas por coma)',
                        validators=[Optional()],
    ...
    def validate_backup_frequency(self, field):
        ...
        if self.backup_required.data and not field.data:
            raise ValidationError(
                'Por favor seleccione una frecuencia de respaldo')
