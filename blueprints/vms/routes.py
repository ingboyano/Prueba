...
def new_vm():
    ...
    if form.validate_on_submit():
        ...
        if form.vlans.data:
            vlans_list = [
                vlan.strip() for vlan in form.vlans.data.split(',')
                if vlan.strip()
            ]
        ...
        vm = VM(name=form.name.data,
                ...
                if form.backup_required.data else None,
                vlans=vlans_list,
    ...
    return render_template('vms/form.html',
                           form=form,
                           title='Nueva Solicitud de VM')
...
def update_status(vm_id):
    ...
    if new_status in ['ACTIVO', 'RECHAZADO', 'PENDIENTE', 'EN PEDIDO']:
        ...
        history = VMHistory(
            ...
            modified_by=current_user.id,
            change_description=
            f"Cambio de estado a '{new_status}' por {current_user.username}")
...
def vm_history(vm_id):
    ...
    history = VMHistory.query.filter_by(vm_id=vm_id).order_by(
        VMHistory.created_at.desc()).all()

    return render_template('vms/history.html',
                           vm=vm,
                           history=history,
                           title=f'Historial de {vm.name}')
